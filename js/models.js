...
class Story {
  ...
  // returns true or false based on whether a story instance is in the current user's favorites
  static isFavorite(story, user) {
    return user.favorites.some(s => (s.storyId === story.storyId));
  }
...
class User {
  ...
  async removeFavorite(storyToRemove) {
    ...
    this.favorites.forEach((story, index) => {
        if (story === storyToRemove) {
